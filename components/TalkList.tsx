...
const TalkList = ({ talks, seeMore = false, filters = ['all', true, true, 'all', true, true] }: { talks: any, seeMore?: boolean, filters?: any }) => {
  ...
  if (filter == 'seminar') filtered = filtered.filter((talk: any) => talk.Type == 'Seminar');
    ...
    filtered = filtered.filter((talk: any) => (talk.Type != 'Seminar' && talk.Type != 'Award'));
    if (!oral) filtered = filtered.filter((talk: any) => (!talk.Type.match('Oral')));
    if (!poster) filtered = filtered.filter((talk: any) => (!talk.Type.match('Poster')));
    if (invitation == 'invited') filtered = filtered.filter((talk: any) => (talk.Invited == 'true' || talk.Symposium == 'true'));
    if (!international) filtered = filtered.filter((talk: any) => (!talk.Type.match('International')));
    if (!domestic) filtered = filtered.filter((talk: any) => (!talk.Type.match('Domestic')));
  }
